...
app.post('/flip', async (request, response) => {
    ...
    // var flip2 = `<button class="button2" name="flip2">flip`
    ...
    if (cards.cards[0].value = "JACK") {
        ...
        renderJack(request, response, "", cards.cards[0].image, cardback, cardback, cardback, cardback, 5, message, flip1)
    }
    else{
        renderJack(request, response, "", cards.cards[0].image, cardback, cardback, cardback, cardback, 4, "")
    }
});

app.post('/flip0', async (request, response) => {
    // var flip1 = `<button class="button1" name="flip1">flip this card</button>\n`
    // deck_id = await backend.getDeck(1)
    // cards = await backend.drawDeck(jdeck.deck_id,5)
    // message = ""
    if (jhand.cards[0].value == "JACK") {
        jscore = turnsleft;
        message = `Congradulations, you have won ${jscore} tokens!`
        renderJack(request, response, "disabled", jhand.cards[0].image, cardback, cardback, cardback, cardback, turnsleft, message)
    }
    else{
        turnsleft -= 1;
        if (turnsleft == 0){
            renderJack(request, response, "disabled", jhand.cards[0].image, cardback, cardback, cardback, cardback, turnsleft, "")
            turnsleft = 3
        }
        else{
            renderJack(request, response, "", jhand.cards[0].image, cardback, cardback, cardback, cardback, turnsleft, "")
        }
    }
});

app.post('/flip1', async (request, response) => {
    // var flip1 = `<button class="button1" name="flip1">flip this card</button>\n`
    // var flip2 = `<button class="button2" name="flip2">flip`
    // deck_id = await backend.drawJacktest()
    // deck_id = await backend.getDeck(1)
    // message = ""
    if (jhand.cards[1].value == "JACK") {
        jscore = turnsleft;
        message = `Congradulations, you have won ${jscore} tokens!`
        renderJack(request, response, "", cardback, jhand.cards[1].image, cardback, cardback, cardback, turnsleft, message)
    }
    else{
        turnsleft -= 1;
        if (turnsleft == 0){
            renderJack(request, response, "disabled", cardback, jhand.cards[1].image, cardback, cardback, cardback, turnsleft, "")
            turnsleft = 3
        }else{
            renderJack(request, response, "", cardback, jhand.cards[1].image, cardback, cardback, cardback, turnsleft, message)
        }
...
app.post('/flip2', async (request, response) => {
    // message = ""
    if (jhand.cards[2].value == "JACK") {
        jscore = turnsleft;
        message = `Congradulations, you have won ${jscore} tokens!`
        renderJack(request, response, "", cardback, cardback, jhand.cards[2].image, cardback, cardback, turnsleft, message)
    }
    else{
        turnsleft -= 1;
        if (turnsleft == 0){
            renderJack(request, response, "disabled", cardback, cardback, jhand.cards[2].image, cardback, cardback, turnsleft, "")
            turnsleft = 3
        }else{
            renderJack(request, response, "", cardback, cardback, jhand.cards[2].image, cardback, cardback, turnsleft, message)
        }}
...
app.post('/flip3', async (request, response) => {
    // message = ""
    if (jhand.cards[3].value == "JACK") {
        jscore = turnsleft;
        message = `Congradulations, you have won ${jscore} tokens!`
        renderJack(request, response, "", cardback, cardback, cardback, jhand.cards[3].image, cardback, turnsleft, message)
    }
    else{
        turnsleft -= 1;
        if (turnsleft == 0){
            renderJack(request, response, "disabled", cardback, cardback, cardback, jhand.cards[3].image, cardback, turnsleft, "")
            turnsleft = 3
        }else{
            renderJack(request, response, "", cardback, cardback, cardback, jhand.cards[3].image, cardback, turnsleft, message)
        }
...
app.post('/flip4', async (request, response) => {
    // message = ""
    if (jhand.cards[4].value == "JACK") {
        jscore = turnsleft;
        message = `Congradulations, you have won ${jscore} tokens!`
        renderJack(request, response, "disabled", cardback, cardback, cardback, cardback, jhand.cards[4].image, turnsleft, message)
    }
    else{
        turnsleft -= 1;
        if (turnsleft == 0){
            renderJack(request, response, "disabled", cardback, cardback, cardback, cardback, jhand.cards[4].image, turnsleft, "")
            turnsleft = 3
        }else {
            renderJack(request, response, "disabled", cardback, cardback, cardback, cardback, jhand.cards[4].image, turnsleft, message)
        }}
...
// Render Get Jack Game Screen

function renderJack(request, response, state, card1, card2, card3, card4, card5, turnsleft, message, button) {
    response.render('jack1.hbs', {
